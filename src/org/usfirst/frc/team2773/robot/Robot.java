...
public class Robot extends TimedRobot {
	...
	public void autoLine() {
		if(driveFromEncoders90 <= 10 * distRate)
			drive(1, 0, 0);
		else
			autoStep++
	}
...
/*
 * :- /hdms `ommmh. `+mmmd/os+-` `:ohmmmmdmmmmmhs/ -+oooo/+sdmmmmbmanmmmmmmmd``
 ...
 * -odmmmmmmmjaredmmmmmmmmh` .dmmmmmmmmmmmmmmdh .-`.+hmmmmmmismmmmmmmmmmms
 * hmmmmmmmmmdhddy+-` `odmmdhshmmmmmthemmmmmmmmmd/ `+shys/ odmmmmdy+-` ``
 * `ymmmmmkhaimmmmmrealmmmmm:. `hhailmmy `-omm- smmmmmmmismmmmmmmmvpmmmmm
 * :mmthemmm` -mm: `mmmmmmmmmbadmmmmmmmmmmmmmyo/` ommorbd/ `/oymd.
 * -mmmsquidmmmmmmmmmmmmyommmmmms `:+/:` `dmds+. `dmmmtastesmmmmmmmd/`hmmmmmd:
 * smm/ ommmmmmgoodmmmmmdo` `oydmmmh- `` `` /mmo :malecmmmmmmmmdo.
 * ``-+syhsohddh- +hddy+hmd+` :mmisn'tmmmmd+. ``.-:+hmy-.-:md+oyyy+`
 * ommmrealmmh+` /mmddddm/ `` `dmmmmmmmy/` -josephd +mmmmmds-` ydunlap .dmmmh+.
 * .herwin` `ymmd/` .hmmmmo. ommmmh/` .smmmmdy/. `:::::::. Mr. Meckling is Max's
 * dad. `:::::::
 */